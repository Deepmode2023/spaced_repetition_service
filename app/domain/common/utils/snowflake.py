...
def snowflake_to_timestamp(_id):
    _id = _id >> timestamp_left_shift  # get our timestamp inside the ID
    _id += tw_epoch  # get current timestamp
...
def generator(worker_id, data_center_id, sleep=lambda x: time.sleep(x / 1000.0)):
    ...
    while True:
        ...
        if last_timestamp > timestamp:
            logger.warning(f"clock is moving backwards. waiting until {last_timestamp}")
